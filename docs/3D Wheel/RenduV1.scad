...
nb_branches = 7;           // Nombre de branches reliant le moyeu au cerceau
...
module branche() {
// La branche part du bord du boitier jusqu'au cerceau
longueur_branche = (diametre_volant / 2) - (boitier_larg / 2) + 15; // +15 pour l'ancrage

// On centre la branche
translate([(boitier_larg / 2) - 5 + (longueur_branche / 2), 0, 0])
    ...
    difference() {
        union() {
            ...
            for (i = [0 : nb_branches - 1]) {
                rotate([0, 0, i * (360 / nb_branches)])
                    branche();
            }
        }
        
        // On soustrait les cavités internes et les trous de fixation
        cavites_boitier();
    }
}
